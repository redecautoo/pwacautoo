...
const VagasEditarVaga = () => {
  const navigate = useNavigate();
  const { vagaId } = useParams<{ vagaId: string }>();
  const { vagas, atualizarVaga, condominios } = useVagas();
  const { showAlert } = useApp();

  const vaga = vagas.find(v => v.id === vagaId);
  const condominio = condominios.find(c => c.id === vaga?.condominioId);
  ...
  useEffect(() => {
    if (vaga) {
      setFormData({
        ...
        status: vaga.status,
        disponivelDe: vaga.disponivelDe || "",
        disponivelAte: vaga.disponivelAte || "",
      });
    }
  }, [vaga]);
  ...
  const handleSubmit = async () => {
    if (!isValid || !datasValidas || !vagaId) return;
    ...
    setSuccess(true);
    showAlert("Vaga Atualizada!", "As alterações na vaga foram salvas com sucesso.", "success");
      ...
      <PageTransition>
        <div className="min-h-screen bg-background flex items-center justify-center p-4">
          ...
          >
            ...
            <h2 className="text-xl font-semibold mb-2">Vaga Atualizada!</h2>
            <p className="text-sm text-muted-foreground">As alterações foram salvas com sucesso.</p>
          </motion.div>
        </div>
      </PageTransition>
    );
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-semibold">Editar Vaga</h1><p className="text-sm text-muted-foreground">{condominio?.nome || "Carregando..."}</p></div>
          </div>
        </header>
        <main className="px-4 py-6">
          <div className="max-w-lg mx-auto space-y-6">
            <div className="flex justify-center"><div className="w-20 h-20 rounded-full bg-primary/20 flex items-center justify-center"><ParkingCircle className="w-10 h-10 text-primary" /></div></div>
            <div className="space-y-4">
              ...
              <div><Label htmlFor="observacao">Observação (opcional)</Label><Textarea id="observacao" placeholder="Ex: Próxima ao elevador..." value={formData.observacao} onChange={(e) => handleChange("observacao", e.target.value)} maxLength={120} className="mt-1.5 resize-none" rows={3} /></div>
            </div>
            <Button onClick={handleSubmit} disabled={!isValid || !datasValidas || isSubmitting} className="w-full h-12 font-bold" size="lg">
              {isSubmitting ? <motion.div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full" animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }} /> : <><Save className="w-4 h-4 mr-2" />Salvar Alterações</>}
            </Button>
          </div>
        </main>
      </div>
