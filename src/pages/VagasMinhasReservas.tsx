...
const VagasMinhasReservas = () => {
  ...
  const handleDelete = () => {
    if (reservaToDelete) {
      ...
      showAlert("Reserva Removida", "A reserva foi excluída da sua lista.", "success");
  ...
  const copyLink = (link: string) => {
    ...
    showAlert("Link Copiado", "O link de acesso foi copiado para sua área de transferência.", "success");
  };
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-semibold">Minhas Reservas</h1><p className="text-sm text-muted-foreground">{minhasReservas.length} reserva(s)</p></div>
          </div>
        </header>
                    ...
                    <motion.div key={reserva.id} className="bg-card border border-border rounded-xl overflow-hidden" variants={staggerItemVariants} custom={index}>
                      <div className="p-4">
                        <div className="flex items-start justify-between">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-muted-foreground">{cond?.nome}</p>
                            <p className="text-xs text-muted-foreground mt-2">{reserva.dataInicio} — {reserva.dataFim}</p>
                            <p className="text-xs font-bold text-foreground mt-1">Placa: {reserva.placa}</p>
                          </div>
                          <div className="text-right space-y-2">
                            <div className="text-lg font-bold text-primary">R$ {reserva.valorTotal}</div>
                            <Button size="icon" variant="ghost" className="text-destructive hover:bg-destructive/10" onClick={() => setReservaToDelete(reserva.id)}><Trash2 className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      </div>
                      {(reserva.status === 'confirmada' || reserva.status === 'pendente') && (
                        <div className="border-t border-border px-4 py-3 bg-secondary/20 flex items-center justify-between">
                          <div className="flex items-center gap-2 text-xs truncate max-w-[200px]"><QrCode className="w-3 h-3 text-primary" /><span className="text-muted-foreground truncate">{reserva.linkAcesso}</span></div>
                          <div className="flex gap-1">
                            <Button size="sm" variant="ghost" onClick={() => copyLink(reserva.linkAcesso)}><Copy className="w-4 h-4" /></Button>
                            <Button size="sm" variant="ghost" asChild><Link to={`/v/${reserva.linkAcesso.split('/').pop()}`}><ExternalLink className="w-4 h-4" /></Link></Button>
                          </div>
                        </div>
                      )}
                    </motion.div>
        ...
        <AlertDialog open={!!reservaToDelete} onOpenChange={() => setReservaToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader><AlertDialogTitle>Excluir Reserva?</AlertDialogTitle><AlertDialogDescription>Deseja remover esta reserva da sua lista?</AlertDialogDescription></AlertDialogHeader>
