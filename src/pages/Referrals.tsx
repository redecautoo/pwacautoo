...
const Referrals = () => {
  ...
  const { currentUser, referrals, showAlert } = useApp();
  ...
  const handleCopy = () => {
    ...
    setCopied(true);
    showAlert("Link Copiado!", "Compartilhe seu link de indicação com amigos e ganhe pontos.", "success");
      ...
      <main className="px-4 py-6">
        <div className="max-w-lg mx-auto space-y-6">
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            ...
            <p className="text-xs text-muted-foreground mt-3">Código: <span className="font-mono text-foreground">{currentUser?.referralCode}</span></p>
          </section>
          ...
          <section className="bg-card border border-border rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-sm text-foreground">Compartilhe seu link</p><p className="text-xs text-muted-foreground">Envie para amigos e família</p></div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-sm text-foreground">Eles se cadastram</p><p className="text-xs text-muted-foreground">Você ganha +5 pontos por cadastro</p></div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div><p className="text-sm text-foreground">Eles compram Selo Azul</p><p className="text-xs text-muted-foreground">Você ganha +10 pontos extras</p></div>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-2">
              {rewards.map((reward, i) => {
                ...
                return (
                  <div key={i} className={`flex items-center justify-between p-4 rounded-xl ${unlocked ? 'bg-primary/10 border border-primary' : 'bg-card border border-border'}`}>
                    <div className="flex items-center gap-3">
                      <Award className={`w-5 h-5 ${unlocked ? 'text-primary' : 'text-muted-foreground'}`} />
                      <div><p className={`text-sm ${unlocked ? 'text-foreground font-medium' : 'text-muted-foreground'}`}>{reward.reward}</p><p className="text-xs text-muted-foreground">{reward.points} pontos</p></div>
                    </div>
                    {unlocked && <span className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded">Desbloqueado</span>}
                  </div>
                );
              })}
            </div>
            {pointsToNextReward > 0 && <p className="text-xs text-muted-foreground text-center mt-4">Faltam {pointsToNextReward} pontos para a próxima recompensa</p>}
          </section>
              ...
              <div className="bg-card border border-border rounded-2xl divide-y divide-border">
                {referrals.map((referral) => (
                  <div key={referral.id} className="flex items-center justify-between p-4">
                    <div><p className="text-sm text-foreground">{referral.referredUserName}</p><p className="text-xs text-muted-foreground">{referral.boughtBlueSeal ? 'Com Selo Azul' : 'Cadastrado'}</p></div>
                    <span className="text-sm font-medium text-green-500">+{referral.pointsEarned}</span>
                  </div>
                ))}
              </div>
            </section>
