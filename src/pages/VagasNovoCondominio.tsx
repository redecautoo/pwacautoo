...
const VagasNovoCondominio = () => {
  ...
  const handleSubmit = async () => {
    ...
    setSuccess(true);
    showAlert("Condomínio Criado!", "O condomínio foi cadastrado com sucesso no sistema.", "success");
      ...
      <PageTransition>
        <div className="min-h-screen bg-background flex items-center justify-center p-4">
          <motion.div className="bg-card border border-border rounded-xl p-8 text-center max-w-sm" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
            ...
            <h2 className="text-xl font-semibold mb-2">Condomínio Criado!</h2>
            <p className="text-sm text-muted-foreground">O condomínio foi cadastrado e agora está visível para moradores.</p>
          </motion.div>
        </div>
      </PageTransition>
    );
      ...
      <div className="min-h-screen bg-background">
        <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
          <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-semibold">Novo Condomínio</h1><p className="text-sm text-muted-foreground">Cadastre seu condomínio</p></div>
          </div>
        </header>
          ...
          <div className="max-w-lg mx-auto space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2"><Label htmlFor="cidade">Cidade *</Label><Input id="cidade" placeholder="Ex: São Paulo" value={formData.cidade} onChange={(e) => handleChange("cidade", e.target.value)} maxLength={100} className="mt-1.5" /></div>
