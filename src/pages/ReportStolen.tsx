...
import { ArrowLeft, AlertTriangle, MapPin, Calendar, Clock, Car, CreditCard, Award, Wallet } from "lucide-react";
...
const ReportStolen = () => {
  ...
  const [usedGreenSealAlert, setUsedGreenSealAlert] = useState(false);
  ...
  // Veículo já roubado - usar SuccessModal com variant warning
  if (vehicle.isStolen) {
    return (
      <PageTransition>
        <div className="min-h-screen bg-background flex items-center justify-center px-6">
          <SuccessModal
            isOpen={true}
            onClose={() => navigate("/stolen")}
            title="Alerta de Roubo Já Ativo"
            description="Este veículo já está com alerta de roubo ativo. Você pode renovar ou visualizar os alertas existentes."
            variant="warning"
            buttonText="Ver Alertas de Roubo"
          />
        </div>
      </PageTransition>
    );
  ...
  const handleRequestPayment = () => {
    if (location && date && time) {
      // Se tem alerta gratuito, pula confirmação de pagamento
      if (hasFreeAlert) {
  ...
  const handleConfirmPayment = () => {
    // Se usou alerta gratuito do selo verde
    if (hasFreeAlert) {
      useGreenSealStolenAlert();
      setUsedGreenSealAlert(true);
    } else {
      const valor = 10;
      
      // Verificar saldo TOTAL (cauCashBalance + saldoVagas)
      if (saldoTotal < valor) {
        toast.error("Saldo insuficiente na CauCash. Recarregue para reportar o roubo.");
  ...
  // Mensagem de sucesso personalizada conforme especificação
  const successTitle = "Alerta de Roubo Ativado!";
  const successDescription = "Seu veículo agora está marcado como roubado na Rede Cautoo. Todos os usuários da rede foram notificados.";

  return (
    <PageTransition>
      <div className="min-h-screen bg-background">
        {/* Confirmação de sucesso - Novo layout */}
        <SuccessModal
          isOpen={showSuccess}
        ...
        <main className="px-4 py-6">
          <div className="max-w-lg mx-auto space-y-6">
            {/* Veículo selecionado - Modo Alerta Máximo */}
            <motion.div
            ...
            >
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-[#D32F2F]/30 flex items-center justify-center">
            ...
            {/* Informação de custo ou alerta gratuito */}
            {hasFreeAlert ? (
              <motion.div
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <p className="font-medium text-green-600 mb-1">🎉 Alerta Gratuito Disponível!</p>
                    <p>
                      Você conquistou <span className="font-bold text-foreground">{freeAlertsRemaining} alerta(s) gratuito(s)</span> com o Selo Verde.
                    </p>
                    <p className="mt-1 text-xs">
                      Este benefício renova a cada 6 meses se você mantiver o Selo Verde ativo.
                    </p>
                  </div>
                </div>
              </motion.div>
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="text-sm text-muted-foreground">
                    <p className="font-medium text-blue-600 mb-1">Custo do Alerta</p>
                    <p>
                      <span className="font-bold text-foreground">R$ 10,00</span> por ativação. O alerta ficará ativo por <span className="font-medium text-foreground">7 dias</span>.
                    </p>
                    <p className="mt-1 text-xs">
                      Após 7 dias, você pode renovar gratuitamente por mais 3 dias (1x).
                    </p>
                  </div>
                </div>
              </motion.div>
            ...
            {/* Aviso importante - Modo Alerta Máximo */}
            <div className="bg-gradient-to-br from-[#D32F2F]/16 to-[#D32F2F]/10 border border-[#D32F2F]/55 rounded-xl p-4 flex items-start gap-3 shadow-[0_0_0_1px_rgba(211,47,47,0.25),0_10px_30px_rgba(0,0,0,0.35)]">
              ...
              <div className="space-y-1">
                <p className="text-sm font-bold text-white">Aviso Importante</p>
                <p className="text-xs text-white/[0.78] leading-relaxed">
                  Ao reportar o roubo, seu veículo será exibido para toda a rede Cautoo. 
                  Usuários verificados poderão enviar avistamentos para ajudar na localização.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <Button
                ...
                className={`w-full py-6 text-white font-bold shadow-[0_0_0_1px_rgba(211,47,47,0.25),0_4px_16px_rgba(211,47,47,0.3)] ${hasFreeAlert 
                  ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700' 
                  : 'bg-[#D32F2F] hover:bg-[#B71C1C] active:bg-[#B71C1C]'
                }`}
                size="lg"
              >
                {hasFreeAlert ? (
                  <>
                    ...
                    Ativar Alerta Gratuito (Selo Verde)
                  </>
                ) : (
                  <>
                    <AlertTriangle className="w-4 h-4 mr-2" />
                    Ativar Alerta de Roubo — R$ 10,00
                  </>
                )}
              </Button>
            </motion.div>
            
            {/* Modal de confirmação de pagamento - Fundo Opaco */}
            {showPaymentConfirm && (
              <motion.div 
                ...
                >
                  <div className="text-center mb-6">
                    ...
                    <p className="text-sm text-gray-300">
                      Este alerta custa <span className="font-bold text-white">R$ 10,00</span> e ficará ativo por <span className="font-bold text-white">7 dias</span>.
                    </p>
                  </div>
                  ...
                  <div className="bg-[#252525] rounded-lg p-3 mb-6 border border-[#D32F2F]/40">
                    ...
                    <div className="flex justify-between text-sm mt-1">
                      ...
                      <span className="font-bold text-[#E53935]">R$ 10,00</span>
                    </div>
                  </div>
