...
const Friends = () => {
  ...
  const handleSendRequest = () => {
    if (plateIsValid && selectedContactReason) {
      ...
      toast({
        ...
        description: `Enviamos seu pedido para a placa ${plateValue}`,
      });
    }
  };
  ...
  const handleAcceptRequest = () => {
    ...
    toast({
      ...
      description: `Informações compartilhadas com sucesso.`,
    });
  };
  ...
  const handlePurchaseAssistance = () => {
    if (totalAssistancePrice > (currentUser?.cauCashBalance || 0)) {
      toast({
        ...
        description: "Você não tem saldo CauCash suficiente para esta compra.",
        variant: "destructive",
      });
      return;
    }
    ...
    toast({ 
      ...
      description: "A cobertura será ativada no período selecionado e o valor foi debitado do seu CauCash." 
    });
  };
      ...
      <div className="min-h-screen bg-background">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    ...
                    contactRequests.map(req => (
                      <div key={req.id} className="bg-card border border-emerald-500/10 rounded-2xl p-4 space-y-4 hover:shadow-lg transition-all border-l-4 border-l-emerald-500">
                        ...
                        <Button 
                          variant="default" 
                          size="sm" 
                          className="w-full h-8 bg-destructive text-white hover:bg-destructive/90 gap-2 text-[10px] font-bold"
                          onClick={() => {
                            if ((req as any).isFromApp) {
                            ...
                            toast({
                              ...
                              description: `${req.fromName} foi bloqueado e denunciado.`,
                              variant: "destructive",
                            });
                          }}
                        >
                        ...
                        <div key={contact.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between hover:bg-emerald-500/5 transition-colors group">
                          ...
                          <div className="flex items-center gap-2">
                            {contact.sharedData.phone && (
                                <Button size="icon" variant="ghost" className="h-8 w-8 text-emerald-500">
                                    <Phone className="w-4 h-4" />
                                </Button>
                            )}
                            <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleDeleteContact(contact.id)}>
              ...
              <TabsContent value="fleet" className="space-y-6 animate-in fade-in-50 duration-300">
                <div className="bg-gradient-to-br from-emerald-600/20 to-teal-600/5 border border-emerald-500/30 rounded-2xl p-6 shadow-md relative overflow-hidden">
                  ...
                  <Button onClick={() => setShowCreateFleet(true)} className="w-full bg-emerald-600 hover:bg-emerald-700 h-11 shadow-lg shadow-emerald-600/20 relative z-10 font-bold">
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-border/40">
                          ...
                          <div className="flex gap-2">
                             <Button variant="ghost" size="icon" className="h-8 w-8 text-emerald-500" onClick={(e) => { e.stopPropagation(); setSelectedFleet(fleet); setShowAddPlateDialog(true); }}>
                                <PlusCircle className="w-4 h-4" />
                             </Button>
                             <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:bg-destructive/10 rounded-full" onClick={(e) => {
                               e.stopPropagation();
        ...
        <Dialog open={!!showAcceptContactDialog} onOpenChange={(open) => !open && setShowAcceptContactDialog(null)}>
          <DialogContent className="max-w-md rounded-3xl">
            ...
            <DialogFooter>
              <Button onClick={handleAcceptRequest} className="w-full bg-emerald-600 hover:bg-emerald-700 h-11">
              ...
              <>
                <div className="bg-emerald-600 p-6 text-white relative">
                  <div className="flex items-center justify-between relative z-10">
                    ...
                    <Button variant="ghost" size="sm" className="text-white hover:bg-white/10" onClick={() => setShowAddPlateDialog(true)}>
                    ...
                    <div className="space-y-2">
                      {selectedFleet.members.map((m: any) => (
                        <div key={m.plate} className="flex items-center justify-between p-3 rounded-xl bg-secondary/20 border border-border/40">
                          ...
                          <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive/60 hover:text-destructive hover:bg-destructive/10" onClick={() => handleDeletePlateFromFleet(selectedFleet.id, m.plate)}>
                            <X className="w-4 h-4" />
                          </Button>
                        </div>
                      ))}
                ...
                <div className="p-4 bg-secondary/30 border-t border-border flex gap-2">
                  ...
                  <Button className="flex-1 rounded-xl bg-emerald-600 hover:bg-emerald-700" onClick={() => setSelectedFleet(null)}>
                    Voltar
                  </Button>
                </div>
              </>
        ...
        <Dialog open={showAddPlateDialog} onOpenChange={setShowAddPlateDialog}>
          <DialogContent className="max-w-md rounded-3xl">
            ...
            <DialogFooter>
              <Button onClick={handleAddPlateToFleet} disabled={!isValidPlate(newPlate)} className="w-full bg-emerald-600 hover:bg-emerald-700 h-11">
                Adicionar Veículo
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={showServiceDialog} onOpenChange={setShowServiceDialog}>
          <DialogContent className="max-w-md rounded-3xl">
            ...
            <DialogFooter>
              <Button onClick={handlePurchaseAssistance} className="w-full bg-emerald-600 hover:bg-emerald-700 h-11" disabled={!startDate || !endDate}>
                Pagar com CauCash
              </Button>
            </DialogFooter>
          </DialogContent>
        ...
        <Dialog open={showCreateFleet} onOpenChange={setShowCreateFleet}>
          <DialogContent className="max-w-md rounded-3xl">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {["Seguro Compartilhado", "Evento ou Viagem", "Frota de Condomínio", "Clube Automotivo", "Outros"].map(type => (
                    <button 
                      ...
                      className={`text-left p-3 rounded-xl border transition-all text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
                        selectedFleetPurpose === type 
                ...
                {selectedFleetPurpose === "Outros" && (
                  <div className="pt-2">
                    <Input 
                      placeholder="Informe a finalidade da frota" 
            ...
            <DialogFooter>
              <Button 
                onClick={() => {
                  if (!newFleetName.trim() || !selectedFleetPurpose) {
                    toast({ title: "Preencha todos os campos", variant: "destructive" });
                    return;
                  }
                  const finalPurpose = selectedFleetPurpose === "Outros" ? customFleetPurpose : selectedFleetPurpose;
                  const newFleet = {
                    id: Date.now().toString(),
                    name: newFleetName,
                    type: selectedFleetPurpose === "Outros" ? "Personalizada" : selectedFleetPurpose,
                    members: [],
                    status: "Ativa",
                    purpose: finalPurpose || selectedFleetPurpose
                  };
                  setMyFleets([...myFleets, newFleet]);
                  setShowCreateFleet(false);
                  setNewFleetName("");
                  setSelectedFleetPurpose("");
                  setCustomFleetPurpose("");
                  toast({ title: "Frota Criada!", description: "Agora você pode adicionar veículos e contratar serviços." });
                }} 
                disabled={!newFleetName.trim() || !selectedFleetPurpose || (selectedFleetPurpose === "Outros" && !customFleetPurpose.trim())}
        ...
        {/* Dialog Cotação Seguro Compartilhado */}
        <Dialog open={showInsuranceQuoteDialog} onOpenChange={setShowInsuranceQuoteDialog}>
          <DialogContent className="max-w-sm rounded-2xl">
            <DialogHeader>
              ...
              <DialogDescription className="pt-2">
                Um especialista Cautoo entrará em contato com a cotação personalizada para a sua frota em até 3 dias úteis.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <Button 
                onClick={() => setShowInsuranceQuoteDialog(false)} 
                className="w-full bg-emerald-600 hover:bg-emerald-700 h-11"
              >
                Entendi
              </Button>
        ...
        <SuccessModal
          ...
          title="Solicitação Enviada!"
          description="Aguarde o dono do veículo"
